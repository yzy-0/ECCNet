...
from mydesignmodel.backbone.p2t import p2t_small
...
class MaxEntropyHintFusion(nn.Module):
    ...
    def _calc_entropy(self, feat):
        ...
        local_entropy = - (prob * torch.log(prob + 1e-10)).sum(dim=1, keepdim=True)  
    ...
    def forward(self, rgb_feat, depth_feat):
        ...
        w_rgb = rgb_global_entropy / total_entropy  
        w_depth = depth_global_entropy / total_entropy  
...
class ECCNet-S(nn.Module):
    def __init__(self, num_class=41, embed_dims=[64, 128, 320, 512]):
        super(Aux_seg, self).__init__()

        self.channels = embed_dims
        self.rgb_d = p2t_small()
    ...
    def forward(self, rgb, dep):
        ...
        CS1 = self.cs1(rgb_list[0], dep_list[0])[0]  
        CS2 = self.cs2(rgb_list[1], dep_list[1])[0]  
        po1 = self.po1(rgb_list[2], dep_list[2])[0]  
        po2 = self.po2(rgb_list[3], dep_list[3])[0]  
